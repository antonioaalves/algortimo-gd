def salsa_optimization(model, days_of_year, workers, working_shift, shift, pessObj, working_days, closed_holidays, min_workers, week_to_days):
    ...
    for w in workers:
        ...
        L_counts[w] = sum(shift.get((w, d, "L"), 0) for d in working_days[w] if (w, d, s) in shift)
        
        # Sum all LQ shifts for this worker
        LQ_counts[w] = sum(shift.get((w, d, "LQ"), 0) for d in working_days[w] if (w, d, s) in shift)
