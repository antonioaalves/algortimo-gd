...
def read_data_salsa(medium_dataframes: Dict[str, pd.DataFrame], algorithm_treatment_params: Optional[Dict[str, Any]] = None) -> Tuple[Any, ...]:
    ...
    try:
        ...
        for w in workers_complete:
            ...
            logger.info(f"Worker {w} empty days after processing: {empty_days[w]}")
            logger.info(f"Worker {w} holiday days after processing: {worker_holiday[w]}")
            logger.info(f"Worker {w} missing days after processing: {missing_days[w]}")
            logger.info(f"Worker {w} fixed days off after processing: {fixed_days_off[w]}")
            logger.info(f"Worker {w} free days in complete cycle after processing: {free_day_complete_cycle[w]}")
            logger.info(f"Closed Holidays after processing: {closed_holidays}")
