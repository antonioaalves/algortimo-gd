...
def holiday_compensation_days(model, shift, workers, working_days, holidays, week_to_days, working_shift, week_compensation_limit,
    ...
    for w in workers:
        ...
        for comp_day in all_possible_comp_days:
            ...
            comp_day_used = model.NewBoolVar(f'comp_day_used_{w}_{comp_day}_hol')
            ...
            for d in worked_holidays[w].keys():
                ...
                if comp_day in possible_compensation_days[w].get(d, []):
                    ...
                    if w not in contingente:
                        contingente[w] = {}
                    contingente[w][(d, comp_day)] = assignment_var
        ...
        for d in worked_holidays[w].keys():
            ...
            comp_day_vars = [
                contingente[w][(d, comp_day)] 
                for comp_day in possible_compensation_days[w].get(d, [])
                if (d, comp_day) in contingente[w]
            ]
    ...
    return contingente, total_worked_holidays_everyone
...
def sunday_compensation_days(model, shift, workers, working_days, sundays, week_to_days, working_shift, week_compensation_limit,
    ...
    for w in workers:
        ...
        for comp_day in all_possible_comp_days:
            ...
            for sunday in worked_sundays[w].keys():
                ...
                if comp_day in possible_compensation_days[w].get(sunday, []):
                    ...
                    if w not in contingente:
                        contingente[w] = {}
                    contingente[w][(sunday, comp_day)] = assignment_var
        ...
        for sunday in worked_sundays[w].keys():
            # Get all variables for compensation days for this special day
            comp_day_vars = [
                contingente[w][(sunday, comp_day)] 
                for comp_day in possible_compensation_days[w].get(sunday, [])
                if (sunday, comp_day) in contingente[w]
            ]
    ...
    return contingente, total_worked_sundays_everyone

def ld_restriction(model, shift, workers, period, ammount_hol, ammount_sun, total_worked_holidays_everyone, total_worked_sundays_everyone, contingente_h, contingente_d, week_compensation):
    if total_worked_holidays_everyone is not None and total_worked_sundays_everyone is not None:
        for w in workers:
            biggest = period[1] + week_compensation.get(w, 2) * 7
            all_assignment_vars = {}
            ...
            if w in total_worked_holidays_everyone and w in total_worked_sundays_everyone:
                model.Add(sum(shift[(w, d, 'LD')] for d in range(period[0], biggest) if (w, d, 'LD') in shift) == total_worked_holidays_everyone[w] * ammount_hol + total_worked_sundays_everyone[w] * ammount_sun)
            elif w in total_worked_holidays_everyone:
                model.Add(sum(shift[(w, d, 'LD')] for d in range(period[0], biggest) if (w, d, 'LD') in shift) == total_worked_holidays_everyone[w] * ammount_hol)
            elif w in total_worked_sundays_everyone:
                model.Add(sum(shift[(w, d, 'LD')] for d in range(period[0], biggest) if (w, d, 'LD') in shift) == total_worked_sundays_everyone[w] * ammount_sun)
            else:
                model.Add(sum(shift[(w, d, 'LD')] for d in range(period[0], biggest) if (w, d, 'LD') in shift) == 0)
