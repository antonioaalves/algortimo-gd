...
def holiday_compensation_days(model, shift, workers, working_days, holidays, week_to_days, working_shift, week_compensation_limit, fixed_days_off, fixed_LQs, worker_absences, vacation_days):
    ...
    for w in workers:
        ...
        LQs = set(fixed_LQs[w])
        for d in [day for day in holidays if day in working_days[w] - off - LQs]:
            ...
            possible_compensation_days[w][d] = compensation_days_calc(special_day_week, off, LQs, worker_absences[w], vacation_days[w],
                                                                 week_to_days, week_compensation_limit.get(w, 2), working_days[w])
    ...
    for w in workers:        
        ...
        for d in worked_holidays[w].keys():
            ...
            if comp_day_vars:
                ...
                model.Add(sum(comp_day_vars) == 1).OnlyEnforceIf(worked_holidays[w][d])
...
def sunday_compensation_days(model, shift, workers, working_days, sundays, week_to_days, working_shift, week_compensation_limit, fixed_days_off, fixed_LQs, worker_absences, vacation_days):
    possible_compensation_days = {}
    worked_sundays = {}
    ...
    for w in workers:        
        ...
        all_possible_comp_days = set()
        # Now collect all possible compensation days for this worker
        for d in worked_sundays[w].keys():  # Use keys from worked_sundays[w] to ensure alignment
            if d in possible_compensation_days[w]:
        ...
        for comp_day in all_possible_comp_days:
            ...
            for special_day in worked_sundays[w].keys():
                # Check if this special day has this compensation day as an option
                if comp_day in possible_compensation_days[w].get(special_day, []):
                    ...
                    # This compensation day is only assigned if the worker worked that special day
                    # Now this is safe because we know special_day is in worked_sundays[w]
                    model.AddImplication(assignment_var, worked_sundays[w][special_day])
        ...
        # For each special day, ensure it gets a compensation day if worked
        for d in worked_sundays[w].keys():
            # Get all variables for compensation days for this special day
            comp_day_vars = [
                all_comp_day_vars[w][(d, comp_day)] 
                for comp_day in possible_compensation_days[w].get(d, [])
            ...
            if comp_day_vars:
                # Normal case: enforce exactly 1 comp day if worked
                model.Add(sum(comp_day_vars) == 1).OnlyEnforceIf(worked_sundays[w][d])
                model.Add(sum(comp_day_vars) == 0).OnlyEnforceIf(worked_sundays[w][d].Not())
            else:
                model.Add(worked_sundays[w][d] == 0)
        # Total worked holidays
