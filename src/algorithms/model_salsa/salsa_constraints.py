...
def ld_restriction(model, shift, workers, period, ammount_hol, ammount_sun, total_worked_holidays_everyone, total_worked_sundays_everyone, enforce_zero_ld_when_no_compensation=True, fixed_ld_count_per_worker=None):
    """
    Enforce total LD count per worker. For España with params 0 we enforce LD==0 (or fixed LD count
    from calendar) so the solver does not use LD as a generic rest day. fixed_ld_count_per_worker
    optional dict: worker -> count of fixed LD days in period (from input calendar); RHS = that count instead of 0.
    """
    fixed_ld = fixed_ld_count_per_worker or {}

    def _add_ld_constraint(w, rhs):
        model.Add(sum(shift[(w, d, 'LD')] for d in range(period[0], period[1] + 1) if (w, d, 'LD') in shift) == rhs)

    def _rhs_no_compensation(w):
        return fixed_ld.get(w, 0)
    ...
    if total_worked_holidays_everyone is not None and total_worked_sundays_everyone is not None:
        for w in workers:
            if w in total_worked_holidays_everyone and w in total_worked_sundays_everyone:
                ...
                _add_ld_constraint(w, _rhs_no_compensation(w))
