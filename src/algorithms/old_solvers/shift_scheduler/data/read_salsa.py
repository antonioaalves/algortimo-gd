...
def read_data_salsa(matriz_calendario, matriz_estimativas, matriz_colaborador):
    ...
    colaboradores = pd.read_csv(matriz_colaborador, index_col=0, sep = ',')
    ...
    for w in workers:
        ...
        if last_registered_day[w] > 0:  # Ensure worker was registered at some point
    ...
    #Max working days consecutively
    # maxi= {}
    # for w in workers:
    #     maxi[w] = colaboradores[colaboradores['MATRICULA'] == w]['Maxi'].values[0]
    ...
    working_shift = ["M", "T"]
    # Objective of workers in a day and shift
    pessObj = {}
    ...
    for d in days_of_year:
        # Compute the correct week number
        week = (d + (5 - start_weekday)) // 7 + 1
        if week not in week_to_days_salsa:
            week_to_days_salsa[week] = []
        week_to_days_salsa[week].append(d)
    ...
    return cal, days_of_year, sundays, holidays, special_days, closed_holidays, empty_days, worker_holiday, missing_days, working_days , non_holidays, start_weekday, week_to_days, worker_week_shift, \
           ...
           df, pessObj, min_workers, max_workers, \
           week_to_days_salsa, week_cut